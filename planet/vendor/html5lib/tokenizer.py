    ...
    * self.stream
    ...
    def consumeEntity(self, allowedChar=None, fromAttribute=False):
        ...
        if fromAttribute:
            ...
            self.tokenQueue.append(
                {"type": tokenTypes["Characters"], "data": output})
    ...
    def tagOpenState(self):
        ...
        if data == u"!":
            ...
            self.tokenQueue.append(
                {"type": tokenTypes["Characters"], "data": u"<>"})
            self.state = self.dataState
        elif data == u"?":
            # XXX In theory it could be something besides a tag name. But
            # do we really care?
    ...
    def rcdataEndTagNameState(self):
        appropriate = self.currentToken and self.currentToken[
            "name"].lower() == self.temporaryBuffer.lower()
        data = self.stream.char()
    ...
    def scriptDataEscapedLessThanSignState(self):
        ...
        if data == "/":
            ...
            self.tokenQueue.append(
                {"type": tokenTypes["Characters"], "data": u"<" + data})
            self.temporaryBuffer = data
    ...
    def markupDeclarationOpenState(self):
        ...
        if charStack[-1] == u"-":
            ...
            if charStack[-1] == u"-":
                self.currentToken = {
                    "type": tokenTypes["Comment"], "data": u""}
    ...
    def doctypeNameState(self):
        data = self.stream.char()
        if data in spaceCharacters:
            self.currentToken["name"] = self.currentToken[
                "name"].translate(asciiUpper2Lower)

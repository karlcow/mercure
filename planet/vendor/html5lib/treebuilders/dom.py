            ...
            if not nsmap:
                ...
                for attrname in node.attributes.keys():
                    ...
                    if (attr.namespaceURI == XMLNS_NAMESPACE or (
                        ...
                        prefix = (
                            attr.nodeName != 'xmlns' and attr.nodeName or None)
                ...
                for attrname in node.attributes.keys():
                    ...
                    if attr.namespaceURI is None and ':' in attr.nodeName:
                        ...
                        if prefix in nsmap:
                            del attributes[(attr.namespaceURI, attr.nodeName)]
                            attributes[(nsmap[prefix], attr.nodeName)
                                       ] = attr.nodeValue

                # SAX events
                ns = node.namespaceURI or nsmap.get(None, None)
                handler.startElementNS(
                    (ns, node.nodeName), node.nodeName, attributes)
                for child in node.childNodes:
                    dom2sax(child, handler, nsmap)

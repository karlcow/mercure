...
class BasicAuthentication(Authentication):
    ...
    def request(self, method, request_uri, headers, content):
        ...
        headers['authorization'] = 'Basic ' + \
            base64.encodestring("%s:%s" % self.credentials).strip()
...
class DigestAuthentication(Authentication):
    ...
    def request(self, method, request_uri, headers, content, cnonce=None):
        ...
        request_digest = '"%s"' % KD(H(self.A1),
                                     "%s:%s:%s:%s:%s" % (self.challenge['nonce'],
...
class HmacDigestAuthentication(Authentication):
    ...
    def request(self, method, request_uri, headers, content):
        ...
        headers['Authorization'] = 'HMACDigest username="%s", realm="%s", snonce="%s", cnonce="%s", uri="%s", created="%s", response="%s", headers="%s"' % (
            self.credentials[0], self.challenge['realm'], self.challenge['snonce'], cnonce, request_uri, created, request_digest, keylist, )
...
the same interface as FileCache."""
    ...
    def _request(
        ...
        if (self.follow_all_redirects or (method in [
                "GET", "HEAD"]) or response.status == 303):
            if self.follow_redirects and response.status in [
                ...
                if redirections:
                    ...
                    if 'location' in response:
                        ...
                        redirect_method = (
                            (response.status == 303) and (
                                method not in [
                                    "GET", "HEAD"])) and "GET" or method
                        (response,
                         content) = self.request(location,
                                                 redirect_method,
                                                 body=body,
                                                 headers=headers,
                    ...
                    raise RedirectLimit(
                        _("Redirected more times than rediection_limit allows."), response, content)
        ...
        try:
            ...
            if conn_key in self.connections:
                ...
                if not connection_type:
                    connection_type = (
                        scheme == 'https') and HTTPSConnectionWithTimeout or HTTPConnectionWithTimeout
            ...
            if method in [
                    "GET",
                    "HEAD"] and 'range' not in headers and 'accept-encoding' not in headers:
            ...
            if cached_value and method in [
                    "GET", "HEAD"] and self.cache and 'range' not in headers:
                if '-x-permanent-redirect-url' in info:
                    # Should cached permanent redirects be counted in our
                    # redirection count? For now, yes.
                    (response,
                     new_content) = self.request(info['-x-permanent-redirect-url'],
                                                 "GET",
                                                 headers=headers,
                                                 redirections=redirections - 1)

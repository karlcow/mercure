...
def foaf2config(rdf, config, subject=None, section=None):
    ...
    for statement in model.find_statements(
            Statement(subject, foaf.weblog, None)):
        ...
        if not title:
            title = model.get_target(statement.object, dc.title)
        if not title:
            continue
        ...
        if accounts.keys():
            for statement in model.find_statements(
                ...
                rdfaccthome = model.get_target(
                    statement.object, foaf.accountServiceHomepage)
                rdfacctname = model.get_target(
                    statement.object, foaf.accountName)

                if not rdfaccthome or not rdfacctname:
                    continue

                if not rdfaccthome.is_resource() or str(rdfaccthome.uri) not in accounts:
                    continue

                if not rdfacctname.is_literal():
                    continue
                ...
                try:
                    servicetitle = rdfaccthome.replace(
                        'http://', '').split('/')[0]
                except:
                    servicetitle = rdfaccthome

                feed = accounts[rdfaccthome].replace(
                    "{foaf:accountName}", rdfacctname)
                if not config.has_section(feed):
                    config.add_section(feed)
        ...
        if depth > 0:
            ...
            for statement in model.find_statements(
                    Statement(person, foaf.knows, None)):
                ...
                # nothing to see
                if not seeAlso or not seeAlso.is_resource():
                    continue
                ...
                try:
                    ...
                    downloadReadingList(
                        seeAlso, config, lambda data, subconfig: friend2config(
                            model, friend, seeAlso, subconfig, data), False)
                except:
                    pass
...
def copy_options(config, parent_section, child_section, overrides={}):
    ...
    for option in [x for x in config.options(
            parent_section) if x in inheritable_options]:

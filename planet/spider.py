...
def writeCache(feed_uri, feed_info, data):
    ...
    if config.activity_threshold(feed_uri):
        updated = sorted([entry.updated_parsed for entry in data.entries
                          if 'updated_parsed' in entry])
    ...
    if data.status == 226:
        if 'planet_message' in data.feed:
